...
function toCard(doc) {
  return {
    ...
    photoUrl: doc.image ? `${BASE}/${doc.image}` : "",
    ...
    status: doc.embedding ? "ai-processed" : "pending-ai",
    statusLabel: doc.embedding ? "AI Processed" : "Pending AI"
  };
}
...
function NewCaseModal({ onClose, onSubmit }) {
  ...
  if (!form.name || !form.location) {
    setErrors({ submit: "Name and location are required" });
    return;
  }
  ...
  try {
    ...
    if (!res.ok) throw new Error(data.message || "Failed to add suspect");

    onSubmit(toCard(data.suspect));
    setSubmitted(true);
